import type { PdpTranscribeStripsRequest } from "@runacademy/shared";
...
export async function POST(request: Request) {
  const body = (await request.json()) as PdpTranscribeStripsRequest;
  ...
  const response = await pdpController.transcribeStrips(body, geminiApiKeyOverride, openAiApiKeyOverride);
