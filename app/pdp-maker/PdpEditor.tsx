...
const WAITING_MINI_GAMES: WaitingGameConfig[] = [
  {
    kind: "sparkPatch",
    title: "빛나는 패치 잡기",
    instruction: "움직이는 빛 패치를 눌러 점수를 올리세요.",
    badge: "터치"
  },
  {
    kind: "mintMatch",
    title: "민트 컬러 찾기",
    instruction: "여러 패치 중 민트색 패치만 누르세요.",
    badge: "컬러"
  },
  {
    kind: "goldMatch",
    title: "골드 컬러 찾기",
    instruction: "골드색 패치만 골라 누르면 콤보가 이어집니다.",
    badge: "컬러"
  },
  {
    kind: "numberOne",
    title: "숫자 1 찾기",
    instruction: "흩어진 숫자 중 1번 패치를 빠르게 찾으세요.",
    badge: "숫자"
  },
  {
    kind: "oddPick",
    title: "홀수 찾기",
    instruction: "숫자 패치 중 홀수만 눌러 점수를 모으세요.",
    badge: "판단"
  },
  {
    kind: "ctaPick",
    title: "CTA 찾기",
    instruction: "상세페이지에서 중요한 CTA 패치를 찾아 누르세요.",
    badge: "카피"
  },
  {
    kind: "checkPick",
    title: "체크 찾기",
    instruction: "체크 표시는 누르고 X 표시는 피하세요.",
    badge: "집중"
  },
  {
    kind: "largestPatch",
    title: "가장 큰 패치",
    instruction: "화면에서 가장 큰 패치를 골라 누르세요.",
    badge: "눈썰미"
  },
  {
    kind: "safeSignal",
    title: "안전 신호",
    instruction: "초록 OK 신호만 누르고 빨간 STOP은 피하세요.",
    badge: "반응"
  },
  {
    kind: "sequenceTap",
    title: "1-2-3 순서",
    instruction: "현재 순서에 맞는 숫자를 차례대로 누르세요.",
    badge: "순서"
  }
];
const WAITING_GAME_SPOTS = [
  { x: 20, y: 28 },
  { x: 48, y: 24 },
  { x: 76, y: 34 },
  { x: 28, y: 68 },
...
function pickWaitingGameConfig() {
  const index = Math.floor(Math.random() * WAITING_MINI_GAMES.length);
  return WAITING_MINI_GAMES[index] ?? WAITING_MINI_GAMES[0];
}

function getWaitingGameSpot(index: number) {
  const spot = WAITING_GAME_SPOTS[index % WAITING_GAME_SPOTS.length] ?? WAITING_GAME_SPOTS[0];
  return {
    x: Math.max(12, Math.min(88, spot.x + (Math.random() - 0.5) * 8)),
    y: Math.max(18, Math.min(78, spot.y + (Math.random() - 0.5) * 8))
  };
}

function shuffleWaitingTargets(targets: WaitingGameTarget[]) {
  return [...targets].sort(() => Math.random() - 0.5).map((target, index) => {
    const spot = getWaitingGameSpot(index);
    return { ...target, ...spot };
  });
}

function createWaitingGameTarget(
  token: number,
  index: number,
  overrides: Pick<WaitingGameTarget, "label" | "color" | "isCorrect"> & Partial<WaitingGameTarget>
): WaitingGameTarget {
  const spot = getWaitingGameSpot(index);
  return {
    id: `${token}-${index}-${overrides.label}`,
    x: spot.x,
    y: spot.y,
    size: overrides.size ?? 48,
    token,
...
function createWaitingGameTargets(kind: WaitingGameKind, token = 0, sequenceStep = 1): WaitingGameTarget[] {
  const nextToken = token + 1;

  switch (kind) {
    case "mintMatch":
      return shuffleWaitingTargets([
        createWaitingGameTarget(nextToken, 0, { label: "민트", color: "#4cb7aa", isCorrect: true }),
        createWaitingGameTarget(nextToken, 1, { label: "블루", color: "#2f6bff", isCorrect: false }),
        createWaitingGameTarget(nextToken, 2, { label: "코랄", color: "#c8474d", isCorrect: false }),
        createWaitingGameTarget(nextToken, 3, { label: "크림", color: "#f4efe6", isCorrect: false })
      ]);
    case "goldMatch":
      return shuffleWaitingTargets([
        createWaitingGameTarget(nextToken, 0, { label: "골드", color: "#d8b65b", isCorrect: true }),
        createWaitingGameTarget(nextToken, 1, { label: "네이비", color: "#1d3748", isCorrect: false }),
        createWaitingGameTarget(nextToken, 2, { label: "민트", color: "#4cb7aa", isCorrect: false }),
        createWaitingGameTarget(nextToken, 3, { label: "그레이", color: "#8f979a", isCorrect: false })
      ]);
    case "numberOne":
      return shuffleWaitingTargets(["1", "2", "3", "4"].map((label, index) =>
        createWaitingGameTarget(nextToken, index, {
          label,
          color: label === "1" ? "#4cb7aa" : "#f4efe6",
          isCorrect: label === "1"
        })
      ));
    case "oddPick": {
      const labels = ["2", "5", "8", "11"];
      return shuffleWaitingTargets(labels.map((label, index) =>
        createWaitingGameTarget(nextToken, index, {
          label,
          color: Number(label) % 2 === 1 ? "#d8b65b" : "#f4efe6",
          isCorrect: Number(label) % 2 === 1
        })
      ));
    }
    case "ctaPick":
      return shuffleWaitingTargets(["IMG", "COPY", "CTA", "FAQ"].map((label, index) =>
        createWaitingGameTarget(nextToken, index, {
          label,
          color: label === "CTA" ? "#4cb7aa" : "#f4efe6",
          isCorrect: label === "CTA",
          size: label === "CTA" ? 58 : 48
        })
      ));
    case "checkPick":
      return shuffleWaitingTargets(["✓", "×", "✓", "×"].map((label, index) =>
        createWaitingGameTarget(nextToken, index, {
          label,
          color: label === "✓" ? "#4cb7aa" : "#c8474d",
          isCorrect: label === "✓"
        })
      ));
    case "largestPatch": {
      const sizes = [42, 52, 66, 48];
      const largest = Math.max(...sizes);
      return shuffleWaitingTargets(sizes.map((size, index) =>
        createWaitingGameTarget(nextToken, index, {
          label: `${index + 1}`,
          color: size === largest ? "#d8b65b" : "#f4efe6",
          isCorrect: size === largest,
          size
        })
      ));
    }
    case "safeSignal":
      return shuffleWaitingTargets(["OK", "STOP", "OK", "STOP"].map((label, index) =>
        createWaitingGameTarget(nextToken, index, {
          label,
          color: label === "OK" ? "#4cb7aa" : "#c8474d",
          isCorrect: label === "OK",
          size: label === "OK" ? 54 : 48
        })
      ));
    case "sequenceTap":
      return shuffleWaitingTargets(["1", "2", "3"].map((label, index) =>
        createWaitingGameTarget(nextToken, index, {
          label,
          color: Number(label) === sequenceStep ? "#d8b65b" : "#f4efe6",
          isCorrect: Number(label) === sequenceStep,
          size: Number(label) === sequenceStep ? 58 : 48
        })
      ));
    case "sparkPatch":
    default:
      return [
        createWaitingGameTarget(nextToken, 0, {
          label: "",
          color: "#d8b65b",
          isCorrect: true,
          icon: "spark",
...
function WaitingMiniGame({ progress }: { progress: number }) {
  const [gameConfig] = useState<WaitingGameConfig>(() => pickWaitingGameConfig());
  const [score, setScore] = useState(0);
  const [streak, setStreak] = useState(0);
  const [misses, setMisses] = useState(0);
  const [sequenceStep, setSequenceStep] = useState(1);
  const [targets, setTargets] = useState<WaitingGameTarget[]>(() =>
    createWaitingGameTargets(gameConfig.kind, 0, 1)
  );
  const gameStateRef = useRef({
    game: gameConfig.title,
    score,
    streak,
    misses,
    sequenceStep,
  ...
  }, [gameConfig.title, misses, score, sequenceStep, streak, targets]);
  ...
  useEffect(() => {
    ...
    const previousRenderGameToText = browserWindow.render_game_to_text;
    const previousAdvanceTime = browserWindow.advanceTime;
    ...
    browserWindow.advanceTime = () => {
      setTargets((currentTargets) =>
        createWaitingGameTargets(gameConfig.kind, currentTargets[0]?.token ?? 0, gameStateRef.current.sequenceStep)
      );
    };

    return () => {
      browserWindow.render_game_to_text = previousRenderGameToText;
      browserWindow.advanceTime = previousAdvanceTime;
    };
  }, [gameConfig.kind]);

  useEffect(() => {
    setTargets((currentTargets) =>
      createWaitingGameTargets(gameConfig.kind, currentTargets[0]?.token ?? 0, sequenceStep)
    );
    const timer = window.setInterval(() => {
      setTargets((currentTargets) =>
        createWaitingGameTargets(gameConfig.kind, currentTargets[0]?.token ?? 0, gameStateRef.current.sequenceStep)
      );
    }, gameConfig.kind === "sequenceTap" ? 1800 : 1250);

    return () => window.clearInterval(timer);
  }, [gameConfig.kind, sequenceStep]);

  const handleTargetClick = (event: ReactMouseEvent<HTMLButtonElement>, target: WaitingGameTarget) => {
    ...
    setScore((currentScore) => currentScore + 10 + Math.min(20, streak * 2));
    setStreak((currentStreak) => currentStreak + 1);
    if (gameConfig.kind === "sequenceTap") {
      setSequenceStep((currentStep) => (currentStep >= 3 ? 1 : currentStep + 1));
      return;
    }
    setTargets((currentTargets) =>
      createWaitingGameTargets(gameConfig.kind, currentTargets[0]?.token ?? 0, sequenceStep)
  ...
  return (
    <div
      ...
      data-game-kind={gameConfig.kind}
    ...
    >
      ...
      <div className={styles.waitingGameTitleRow}>
        <div>
          ...
          <p>
            {gameConfig.kind === "sequenceTap"
              ? `다음 순서: ${sequenceStep}`
              : gameConfig.instruction}
          </p>
        </div>
        <span>{gameConfig.badge}</span>
      </div>
      <div className={styles.waitingGameBoard} onClick={handleMiss}>
        <div className={styles.waitingGameProgressGlow} />
        {targets.map((target) => (
          <button
            aria-label={`${gameConfig.title} ${target.label || "빛나는 패치"}`}
            className={styles.waitingGameTarget}
            data-correct={target.isCorrect ? "true" : "false"}
            data-game-target="true"
            key={target.id}
            onClick={(event) => handleTargetClick(event, target)}
            style={{
              "--waiting-game-target-color": target.color,
              left: `${target.x}%`,
              top: `${target.y}%`,
              width: target.size,
              height: target.size
            ...
            type="button"
          >
            {target.icon === "spark" ? <Sparkles size={20} /> : <span>{target.label}</span>}
          </button>
        ))}
      </div>
    </div>
